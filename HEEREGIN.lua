...
-- // 2. MOBILE OPEN/CLOSE BUTTON (SAFE PARENTING) // --
local function CreateToggleButton(window)
    ...
    Button.Image = "rbxassetid://10110319532" -- Menu Icon
    Button.Position = UDim2.new(0.8, 0, 0.1, 0) -- Top Right
    Button.ZIndex = 100
...
local function IsInAncientDungeon()
    if Workspace:FindFirstChild("MAP") and 
       Workspace.MAP:FindFirstChild("Trial") and 
       Workspace.MAP.Trial:FindFirstChild("Trial - Zone") and 
       Workspace.MAP.Trial["Trial - Zone"]:FindFirstChild("Trial of Ancient") then
        return true
    end
    return false
end
...
local function Kill_Mob_Daguba(Target)
    ...
    Sticker = RunService.Heartbeat:Connect(function()
        ...
        -- FIX: REMOVED "if not _G.IsTransforming" check. Stick ALWAYS.
    ...
    while _G.AutoFarm and Target and Target.Parent == LIVES_FOLDER do
        ...
        -- We still check transforming for combat to avoid breaking animation, but movement continues above
...
local function Run_Daguba_Sequence()
    ...
    if TrialFolder then
        ...
        local Targets = {
            {Name = "Child [3]", Object = Children[3]},
            {Name = "Trial.Part", Object = TrialFolder:FindFirstChild("Part")},
            {Name = "Child [2]", Object = Children[2]}
        }
        ...
        for i, Item in ipairs(Targets) do
            ...
            if Target then
                ...
                warn("⚠️ Target " .. Item.Name .. " missing.")
            end
        end
    end
end
...
local function GetDagubaQuestStatus()
    ...
    if PlayerGui then
        ...
        if QuestFrame and QuestFrame.Visible then
            ...
            if Label and string.find(Label.Text, "Completed") then
                return "COMPLETED" 
            end
            return "ACTIVE" 
        end
...
local function Accept_Daguba_Quest()
    ...
    if NPC_Dojo then
        ...
        if Root then
            ...
            Fluent:Notify({Title = "Daguba", Content = "Submitting / Accepting Quest...", Duration = 3})
            ...
            while _G.AutoFarm and GetDagubaQuestStatus() ~= "ACTIVE" and MaxTries < 3 do
                local steps = {
                    ...
                    {Choice = "Can I get another quest?"},   
                    {Choice = "Sure.."},                     
                    {Choice = "Ancient Argument"},           
                    {Choice = "Start 'Ancient Argument'"},   
                    {Exit = true}                            
                }
...
local function Farm_Agito_Quest()
    if _G.IsTransforming or not _G.AutoFarm then return end
    if Check_Agito_Quest_Active() == "ACTIVE" then return end -- If actively farming, skip. If completed, run logic.
    ...
    if NPC_Agito then
        ...
        if AgitoPart then
            ...
            while _G.AutoFarm and Check_Agito_Quest_Active() ~= "ACTIVE" and MaxTries < 3 do
                DIALOGUE_EVENT:FireServer({Choice = "Yes, I've completed it."}) -- Hand in
                task.wait(0.3)
                DIALOGUE_EVENT:FireServer({Choice = "Can I get another quest?"}) -- Re-take
                task.wait(0.3)
...
local function KillEnemy(EnemyName)
    ...
    local Target = LIVES_FOLDER:FindFirstChild(EnemyName)
    local ticks = 0
    local MaxTicks = (EnemyName == "Agito Lv.90") and 10 or 5 
    while not Target and _G.AutoFarm and ticks < MaxTicks do
        Target = LIVES_FOLDER:FindFirstChild(EnemyName)
        ticks = ticks + 1
        task.wait(1)
    end
    ...
    if Target and Target:FindFirstChild("Humanoid") and Target:FindFirstChild("HumanoidRootPart") then
        ...
        Sticker = RunService.Heartbeat:Connect(function()
            ...
            if _G.AutoFarm and Target and Target.Parent == LIVES_FOLDER and Target:FindFirstChild("HumanoidRootPart") and Target.Humanoid.Health > 0 then
                -- FIX: REMOVED "if not _G.IsTransforming" check here too. Always stick.
        ...
        while _G.AutoFarm and Target.Parent == LIVES_FOLDER do
            if not LIVES_FOLDER:FindFirstChild(EnemyName) then break end
...
local YuiSection = Tabs.Main:AddSection("AUTO QUEST")

local QuestDropdown = Tabs.Main:AddDropdown("QuestSelect", {
    Title = "Select Quest",
    Values = {"YUI (Lv. 1-35)", "AGITO (Shoichi)", "DAGUBA (Auto Dungeon)"},
    Multi = false,
    Default = 1,
})
            ...
            while _G.AutoFarm do
                if not _G.IsTransforming then
                    ...
                    elseif QuestDropdown.Value == "AGITO (Shoichi)" then
                        -- MODIFIED: Check status first.
                        local AgitoStatus = Check_Agito_Quest_Active() -- Returns "ACTIVE" (true if visible), "COMPLETED" (false if text matched), "NONE"
                        
                        if _G.AutoQuest then
                            -- Logic inside Farm_Agito_Quest handles "Completed" vs "None" vs "Active" internally now
                            Farm_Agito_Quest() 
                            
                            -- If active, we proceed to farm
                            if Check_Agito_Quest_Active() == "ACTIVE" or Check_Agito_Quest_Active() == true then
                                if not _G.AutoFarm then break end
                                Summon_Agito()
                                if not _G.AutoFarm then break end
